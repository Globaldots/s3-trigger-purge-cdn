...
# place the API user, password, base url and platform into config_akamai.py
import config_akamai as config
    
assert config.user ,"Error: missing API user"
assert config.password, "Error: missing API password"
assert config.base_url, "Error: missing base URL"
assert config.platform in ['production', 'staging'], "Error: %r is wrong platform, must be production or staging" % (config.platform)
assert (config.lambda_region in ['us-east-2', 
                                ...
                                'eu-west-2', 
                                'sa-east-1' 
                                ]), "Error: bad or missing AWS region"
...
try: 
    boto_response=aws_lambda.create_function(
        ...
        Description='Auto purge akamai from S3 bucket updates',
        Timeout=300,
        MemorySize=memory,
        Publish=True,
        Environment={
